...
// codex/src/cli.ts
import { realpathSync as realpathSync3, unlinkSync as unlinkSync3 } from "node:fs";
...
var cap = (text2) => text2.charAt(0).toUpperCase() + text2.slice(1);
function resetText(named, capital = false) {
  const five = named.find((n) => n.kind === "five_hour");
  const week = named.find((n) => n.kind === "seven_day");
  let text2;
  if (five === void 0 || week === void 0) text2 = oneReset(five ?? week ?? { kind: "five_hour", test: false });
  else if (five.test && week.test) text2 = "the test windows ended";
  else if (!five.test && !week.test) text2 = "the 5-hour and weekly windows reset";
  else text2 = `${oneReset(five)} and ${oneReset(week)}`;
  return capital ? cap(text2) : text2;
}
...
var isoMs = (text2) => /^\d{4}-\d{2}-\d{2}T/.test(text2) ? Date.parse(text2) : Number.NaN;
...
function parseConsent(raw) {
  ...
  const bare = /\s/.test(text2) ? Number.NaN : isoMs(text2);
...
function seeLock(lockFile) {
  ...
  try {
    ...
    const text2 = buf.subarray(0, n).toString("utf8");
    let holder;
    try {
    ...
    return { dev: Number(st.dev), ino: Number(st.ino), mtimeMs: st.mtimeMs, text: text2, holder };
  } finally {
    closeSync(fd);
  }
}
...
function withLock(lockFile, owner, fn, o = {}) {
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    const text2 = JSON.stringify(holder);
...
function tryLock(lockFile, owner, o = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const text2 = JSON.stringify({ owner, pid: process.pid, at: Date.now(), nonce: randomBytes(6).toString("hex") });
    const fd = createLock(lockFile);
    if (fd !== void 0) {
      try {
        writeSync(fd, text2);
...
function cwdOf(transcript) {
  if (typeof transcript !== "string" || transcript === "") return void 0;
  try {
    const line = firstLine(transcript);
    if (line === void 0) return void 0;
    const o = JSON.parse(line);
    const p = isObject2(o) && isObject2(o["payload"]) ? o["payload"] : void 0;
    return p !== void 0 && typeof p["cwd"] === "string" ? p["cwd"] : void 0;
  } catch {
    return void 0;
  }
}
function listSessions(paths, since) {
  ...
  for (const sid of names) {
    ...
    rows.push(cwd === void 0 ? { sid, mtime } : { sid, mtime, cwd });
  }
...
function stopNotice(q, plan, written, via, now, auto) {
  ...
  const text2 = (limit, stopped) => via === "time limit" ? { text: limit } : via !== "command" ? { text: stopped } : {};
  const ended = plan.ended;
  if (ended !== void 0 && late.length === 0 && auto && work && plan.until <= now) {
    return { ...text2(notice.holdLimitLate(facts, ended, true), notice.stoppedLate(facts, ended, true)), late: { record: written, ended, until: u } };
  }
  ...
  return { ...text2(notice.holdLimit(facts, a), notice.stopped(facts, a)), late: { record: written, until: u } };
}
...
function simulateOpens(t, real, reserve, span, now, f) {
  ...
  return { at: clockText(start, f.kind ?? "five_hour", void 0, now), lead: leadText(f) ?? "" };
}
...
function threadIds(store) {
  try {
    return readdirSync2(join2(store.dir, "threads")).filter((n) => n.endsWith(".json")).map((n) => n.slice(0, -".json".length)).sort();
  } catch (e) {
    const code = e.code;
...
function createQuota(d) {
  const livePath = join3(d.paths.data, "live.json");
  const liveLock = join3(d.paths.data, "live.lock");
  const errorPath = join3(d.paths.data, "live-error.json");
  const seedPath = join3(d.paths.data, "seed.json");
  const seedLock = join3(d.paths.data, "seed.lock");
...
// codex/src/stop.ts
function noticeIn(st, text2, now, tag) {
  st.notices = [...st.notices ?? [], { at: now, text: text2, ...tag === void 0 ? {} : { tag } }];
}
...
var configPath = (paths) => join4(paths.data, "config.json");
...
function setOption(paths, owner, name, value) {
  ...
  return withLock(join4(paths.data, "config.lock"), owner, () => {
...
function scratchStore(data) {
  ...
  try {
    const live = readJson(join5(data, "live.json"));
...
function realish(p) {
  ...
  for (; ; ) {
    ...
    if (up === head) return join6(head, ...tail);
...
function findPaths(env, selfFile) {
  ...
  if (set(env.SPARE10_CODEX_DATA)) {
    ...
    codexHome = set(env.CODEX_HOME) ? homeOfEnv(env.CODEX_HOME, env, pluginRoot) : homeFromPath(env.PATH) ?? homeFromPluginRoot(pluginRoot) ?? join6(home, ".codex");
    data = join6(codexHome, "plugins", "data", DATA_NAME);
  }
  const socket = join6(codexHome, "app-server-control", "app-server-control.sock");
  guardTestPath(env, "CODEX_HOME", codexHome);
  guardTestPath(env, "data dir", data);
  guardTestPath(env, "daemon socket", socket);
  const bin = join6(data, "bin");
  return { codexHome, data, pluginRoot, socket, launcher: join6(bin, "spare10"), bin, home };
}
    ...
    try {
      for (; ; ) {
        ...
        if (f.opcode === OP.ping) send(OP.pong, f.payload);
          ...
          const text2 = joiner.add(f);
          if (text2 !== void 0) onText(text2);
...
function udsDaemon(paths, version, clock, o = {}) {
  ...
  return {
    ...
    start: (threadId, text2) => op(START_MS, async (c) => {
      const r = await c.request("turn/start", { threadId, input: [{ type: "text", text: text2, text_elements: [] }] });
...
var logFileOf = (dataDir, at) => join7(dataDir, "log", `broker-${new Date(at).toISOString().slice(0, 10)}.log`);
  ...
  return {
    debug(line) {
      try {
        ...
        mkdirSync2(join7(dataDir, "log"), { recursive: true, mode: 448 });
...
function createQuestions(d) {
  ...
  const settle = async (sx, key, outcome, via, o = {}) => {
    ...
    const r = sx.store.locked((tx) => {
      ...
      if (plan.kind === "open") {
        const text2 = stopOpenNotice(q, plan.ended, via);
        if (text2 !== void 0) noticeIn(tx.state, text2, now, "stop");
...
function metaOf(line) {
  ...
  try {
    const o = JSON.parse(line);
    const p = isObject7(o) && isObject7(o["payload"]) ? o["payload"] : void 0;
...
function turnStartOf(line) {
  ...
  const p = isObject7(o) && isObject7(o["payload"]) ? o["payload"] : void 0;
  if (!isObject7(o) || p === void 0 || o["type"] !== "event_msg" || p["type"] !== "task_started") return void 0;
  const turnId = p["turn_id"];
...
function parseLine(c, line, fresh) {
  ...
  if (start !== void 0) {
    if (c.turnStart === void 0 || !(start.at < c.turnStart.at)) c.turnStart = start;
    return;
  }
  if (c.meta === void 0) {
    const m = metaOf(line);
  ...
  for (; ; ) {
    ...
    for (let i = part2.lines.length - 1; i >= 0 && !found; i -= 1) {
      ...
      if (c.turnStart === void 0) {
        const start = turnStartOf(line2);
        if (start !== void 0) c.turnStart = start;
      }
    }
...
function createInterrupts(d) {
  ...
  const keyOf = (thread, turn) => `${thread}\0${turn}`;
  ...
  return {
    async interrupt(sx, thread, turn, o = {}) {
      const join10 = o.join !== false;
      const key = keyOf(thread, turn);
      if (done.has(key)) return join10 ? "joined" : "skipped";
      const running = inflight.get(key);
      if (running !== void 0) return join10 ? await running ? "joined" : "failed" : "skipped";
      let daemon;
      try {
        daemon = d.daemon.get();
      } catch (e) {
      ...
      if (mark.kind === "taken" && !join10) return "skipped";
      const p = (async () => {
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    const a = argv[i] ?? "";
    const eq = a.indexOf("=");
    const flag = a.startsWith("--") ? eq > 0 ? a.slice(0, eq) : a : void 0;
    if (flag === "--full" && eq < 0) {
      out.full = true;
      continue;
    }
  ...
  if (!VERBS.includes(verb)) return void 0;
  if (out.full && verb !== "status") return void 0;
  if ((verb === "status" || verb === "help" || verb === "resume" || verb === "stop") && rest.length > 0) return void 0;
  if (verb === "simulate" && rest.length === 0) return void 0;
...
function commandOf(a) {
  ...
  if (a.verb === "set") {
    ...
    if (o === void 0) return { verb: "unknownOption", word: a.words[0] ?? "" };
    const value = a.words.slice(1);
    return { verb: "set", words: [o.name, ...value], rest: value.join(" ") };
  }
  return { verb: a.verb, words: [], rest: "" };
}
function pathsOf(env, a, selfFile) {
  const e = { ...env };
  if (a.codexHome !== void 0) e.CODEX_HOME = resolve2(a.codexHome);
  if (a.data !== void 0) e.SPARE10_CODEX_DATA = resolve2(a.data);
  return findPaths(e, selfFile);
}
async function main(argv, d) {
  const print = (text2) => void d.stdout.write(`${text2}
`);
  const a = parseArgs(argv);
  if (a === void 0) {
    print(codexText.cliUsage);
    return 2;
  }
  const verb = a.verb;
  const agent = set2(d.env.CODEX_SANDBOX) || d.env.CODEX_SANDBOX_NETWORK_DISABLED === "1";
  if (agent && verb !== "help") {
  ...
  if (verb !== "help") {
    try {
      ...
      const probe = join9(paths.data, `.cli-${pid}.probe`);
      writeFileAtomic(probe, "");
      unlinkSync3(probe);
    } catch (e) {
      const code = codeOf2(e);
  ...
  const fromBang = set2(d.env.CODEX_THREAD_ID);
  const named = a.session ?? (set2(d.env.CODEX_SESSION_ID) ? d.env.CODEX_SESSION_ID : void 0);
  const alive = d.pidAlive ?? pidAlive;
  const wake = d.wake ?? { watch: () => () => {
  }, fire() {
  } };
  const owner = `cli-${pid}`;
  const env = { ...d.env, SPARE10_SIMULATE: void 0 };
  const partsFor = (sid) => {
    const state = sid === void 0 ? void 0 : sessionStore(paths, sid, owner, wake, { clock: d.clock }).read();
    ...
    const attendance = {
      attended(c, mode) {
        ...
        return state?.attended === void 0 ? got : { ...got, attended: state.attended };
      }
    };
    ...
    if (sid === void 0) return { cmds, sx: void 0 };
    const sx = {
      sid,
      thread: sid,
      root: true,
  ...
  try {
    ...
    if ((verb === "resume" || verb === "stop" || verb === "simulate") && named === void 0) {
      ...
      for (const r of listSessions(paths, now - RECENT_MS)) {
        ...
        try {
          ...
          if (p2.sx !== void 0) phase = await p2.cmds.phase(p2.sx);
        } catch {
          phase = "unknown";
        }
        rows.push([r.sid, r.cwd === void 0 ? "-" : shownPath(r.cwd, paths.home), phase]);
      }
    ...
    if (fromBang && changes && p.sx !== void 0) {
...
function isMain() {
  ...
  try {
    return realpathSync3(script) === realpathSync3(fileURLToPath(import.meta.url));
  } catch {
    return false;
  }
}
