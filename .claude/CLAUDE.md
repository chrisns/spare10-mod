...
- `SPARE10_STOPPED` is `<session id> <untilMs> <atMs> <tags>`. The tags are a comma list of kinds (`five_hour`, `seven_day`) and `work`, `auto`, `test`, `skip`. `skip` means that the until is a skip start: no margin. A 0.1 value (three tokens) still stops the 5-hour window, and never continues or extends.
